...
function Vehicle() {
  ...
  function getAllApi() {
    axios
      .get("http://localhost:5000/vehicle/vehicle")
      .then((response) => {
        setContent(response.data.vehicles);
      })
      .catch((error) => {
            ...
            {content.map((item) => {
              return (
                <TR key={item.id}>
                  ...
                  <TD data-label="vehicle Name">{item.vehicle_name}</TD>
                  <TD data-label="own">{item.vehicle_ownership}</TD>
                  <TD data-label="branch Name">{item.branch_name}</TD>
                  <TD data-label="vehicle Number">{item.vehicle_number}</TD>
