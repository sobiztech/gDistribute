...
function AddReturnType() {
  ...
  const formSchema = Yup.object().shape({
    status: Yup.string().required("category is mendatory")
  });
  ...
  const onSubmit = (data) => {
    ...
    if (typeof id === "undefined") {
      axios
        .post("http://localhost:5000/return/return-status", data)
        .then((response) => {
  ...
  return (
    <div style={{ padding: "0px" }}>
      <HeaderAdd
        ...
        name={
          typeof id === "undefined"
            ? NAVIGATION.ADDRETURNTYPE
            : NAVIGATION.EDITRETURNTYPE
        }
        to={NAVIGATION.RETURNTYPE}
      />
