...
function DiscountType() {
  ...
  function getAllApi() {
    axios
      .get("http://localhost:5000/discount/discount-type")
      .then((response) => {
        setContent(response.data.discountType);
        // console.log(response.data.discountType);
      })
      .catch((error) => {
  ...
  return (
    <React.Fragment>
      <HeaderAdd
        btn="ADD"
        name={NAVIGATION.DISCOUNTTYPE}
        to={NAVIGATION.ADDDISCOUNTTYPE}
      />
              ...
              return (
                <TR key={item.id}>
                  ...
                  <TD data-label="action">
                    <ACTION>
                      <Link to={`/discount/discount-type/edit/${item.id}`}>
                        <EDITICON />
                      </Link>
                      <div>
