...
function RRoute() {
  ...
  function getAllApi() {
    axios
      .get("http://localhost:5000/shops/route")
      .then((response) => {
        setContent(response.data.rRoutes);
      })
      .catch((error) => {
            ...
            {content.map((item) => {
              return (
                <TR key={item.id}>
                  ...
                  <TD data-label="Property Name">{item.route_name}</TD>
                  <TD data-label="description">{item.description}</TD>
                  <TD data-label="action">
                    <ACTION>
                      <Link to={`/shop/routes/edit/${item.id}`}>
                        <EDITICON />
                      </Link>
                      <div>
