...
function ShopAuth() {
  ...
  function getAllApi() {
    axios
      .get("http://localhost:5000/shops/shop-auth")
      .then((response) => {
        setContent(response.data.shopAuth);
      })
      .catch((error) => {
  ...
  return (
    <React.Fragment>
      <HeaderAdd
        btn="ADD"
        name={NAVIGATION.SHOPAUTH}
        to={NAVIGATION.ADDSHOPAUTH}
      />
      <div style={{ padding: "10px" }}>
        <TABLES>
          <THEAD>
            <TR>
              <TH scope="col">id</TH>
              <TH scope="col">Shop auth Name</TH>
            ...
            {content.map((item) => {
              return (
                <TR key={item.id}>
                  ...
                  <TD data-label="Shop auth Name">{item.authority_first_name}</TD>
