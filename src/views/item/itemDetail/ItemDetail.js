...
function ItemDetail() {
  ...
  function getAllApi() {
    axios
      .get("http://localhost:5000/items/item-detail")
      .then((response) => {
        setContent(response.data.itemDetail);
      })
      .catch((error) => {
  ...
  return (
    <React.Fragment>
      <HeaderAdd
        btn="ADD"
        name={NAVIGATION.ITEMDETAIL}
        to={NAVIGATION.ADDITEMDETAIL}
      />
              ...
              return (
                <TR key={item.id}>
                  ...
                  <TD data-label="action">
                    <ACTION>
                      <Link to={`/items/item-detail/edit/${item.id}`}>
                        <EDITICON />
                      </Link>
                      <div>
